...
shutil.copytree(os.path.join("website","auth"),os.path.join("docs","auth"),dirs_exist_ok=True)

navbar_html = open(os.path.join("website","navbar.html"), "r", encoding="utf-8").read()
colormode_html = open(os.path.join("website","colormode.html"), "r", encoding="utf-8").read()

for file in os.listdir("website"):
    if ".html" in file and "navbar.html" not in file and "colormode.html" not in file:
        ...
        webpage = BeautifulSoup(webpage.replace("{colormodes}",colormode_html), 'html.parser').prettify()
